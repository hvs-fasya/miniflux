...
// 2018-05-04 17:52:35.448435578 +0300 MSK m=+0.003120726
...
{{ end }}
`,
	"feed_filters": `{{ define "title"}}{{ .feed.Title }} ({{ .total }}){{ end }}

{{ define "content"}}

<a href="#" title="Remove" id="chip-template" hidden class="category" data-on-click="rmFromFilter">
    <input disabled type="hidden" name="filters[]">
    <span title="remove" data-on-click="rmFromFilter"> &#10006;</span>
</a>
...
{{ end }}
    ...
    <label for="filter">{{ t "New filter" }}</label>
...
{{/*<form action="{{ route "filters" }}" method="get" autocomplete="off">*/}}
    {{/*<input type="hidden" name="csrf" value="{{ .csrf }}">*/}}
...
{{/*{{ end }}*/}}
    ...
    {{/*<label for="filter">{{ t "Set filter" }}</label>*/}}
    {{/*<input type="text" name="filter" id="filter" value="{{ t .filter }}" required autofocus>*/}}

    {{/*<div class="buttons">*/}}
        {{/*<button type="submit" class="button button-primary" data-label-loading="{{ t "Loading..." }}">{{ t "Find" }}</button>*/}}
    {{/*</div>*/}}
{{/*</form>*/}}
...
var templateViewsMapChecksums = map[string]string{
	...
	"feed_filters":        "658805f40d9afd2ee3f9b4b62c7dae84667ba6f9ae2cab622b200c42806a4ca4",
